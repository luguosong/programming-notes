...
3. **锚点**：Zensical 使用 Python-Markdown 默认 toc slugify（`unicode=False`），会 `encode('ascii', 'ignore')` 剥离所有非 ASCII 字符（中文、Emoji 全部消失），仅保留 ASCII 字母/数字/连字符，重复 slug 追加 `_1`、`_2`…（H1 也参与计数）。**务必用脚本模拟 `slugify` + `unique` 计算真实锚点**，不可手动猜测
