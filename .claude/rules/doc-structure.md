...
每篇文档的标题必须**三处保持完全一致**：`zensical.toml` nav 配置的标题、front matter 的 `title:`、正文的一级标题（`#`）。三者内容逐字相同，缺一不可。

- **正文必须有一级标题**：每篇 `index.md` 正文开头必须以 `#` 起一个一级标题，且全文仅此一个一级标题，不得省略，也不得用更高层级（如 `##`）顶替。
- Zensical 会优先显示 nav 配置的标题，三处不一致会导致 SEO 标题、页面标题、TOC 入口各不相同，搜索索引混乱。

- ✅ nav: `{ "自定义指令" = "..." }` → front matter: `title: 自定义指令` → 正文: `# 自定义指令`
- ❌ nav: `{ "插件" = "..." }` → front matter: `title: 插件系统`（多了"系统"二字）
- ❌ 正文缺失一级标题，或一级标题文本与 nav 标题不符（如 nav 是「插件」，正文写成 `# 插件系统详解`）
