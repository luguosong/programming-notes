...
Willie Wheeler 和 Joshua White 合著的`《Spring实战》`「用户管理」（Manning, 2013）。这一章详细描述了如何用 Spring 3 编写一个 Web
...
一旦我们开始第一个项目，就会更详细地讨论各种身份认证的方案。在第3到「实现身份认证」中，我们将针对本例中涉及的不同职责，继续介绍更具体的配置方法。你还会看到，根据不同的架构风格，可以用不同的方式应用这些配置。本章我们将讨论以下几个步骤：
...
在本节中，你将学习如何配置 UserDetailsService 和 PasswordEncoder。这两个组件通常参与认证过程，大多数应用程序会根据自身需求对其进行定制。虽然我们将在「用户管理」和「密码管理」详细讨论如何自定义它们，但现在了解如何接入自定义实现同样非常重要。本章中用到的实现均由 Spring Security 提供。
...
!!! note
	...
	了解单元测试和集成测试的重要性后，有些读者可能会好奇，为什么我们没有为示例代码编写测试。其实，本书中提供的所有示例都配套有相关的 Spring Security 集成测试。不过，为了让大家能够专注于每一章节所讲解的核心内容，我将关于 Spring Security 集成测试的相关讨论单独放在了「测试安全配置」进行详细讲解。

### 在端点级别实施授权

随着用户管理方式的更新（详见第2.3.1节），我们现在可以讨论终端的认证方式和配置了。关于授权配置，你将在第7到「在方法级别实现过滤」学到很多内容。但在深入细节之前，你需要先了解整体框架。最好的方式就是通过我们的第一个示例。默认配置下，所有终端都假定你拥有由应用程序管理的有效用户。同时，应用默认采用HTTP
...
各个元素的自定义配置，比如认证、授权，或者特定的安全机制（如 CSRF 或 CORS，这些内容将在「配置CSRF防护」和「配置CORS」中讨论）。
...
本例旨在让你了解如何自定义默认配置。关于授权的具体内容，我们将在第7到「配置CORS」详细介绍。
...
由于这是第一个示例，我只为你展示一个简要的示意图，帮助你更好地理解架构中各个组件之间的关系。接下来的第3到「实现身份认证」，我们会进行更深入的讲解。
...
supports() 方法的具体用法，我会在「实现身份认证」详细讲解。现在，你可以暂时不用关心它的实现，对于当前的示例来说并不是关键。
