...
- 正因为 SecurityContext 的变化，任何授权配置都会受到影响。回想「一个Web应用的安全性始于过滤器」的内容，授权规则通常依赖于保存在 SecurityContext 中的
  Authentication 实例。现在，无论是在端点层应用的安全配置，还是全局方法级别的安全功能都会受到影响。
- UserDetailsService 作为负责获取用户详情的组件，本质上是一个数据源。因此，它也需要支持响应式方式。（我们在「Spring Security 入门」已经学习过这个契约。）
...
在许多应用中，用户的认证方式通常基于一对用户名与密码凭证。这种方式很基础，我们也已经讲过，从「Spring Security 入门」实现的最简单应用开始。但在响应式应用中，负责用户管理的组件实现也会发生变化。本节我们将讨论在响应式应用中实现用户管理。
...
实现将用户信息存储在内存中（与「Spring Security 入门」介绍的 InMemoryUserDetailsManager 相同）。我们需要修改 ssia-ch17-ex1 项目的 pom.xml
...
我在本节前面就提到过，在响应式应用的端点授权配置中也可以像在非响应式应用中一样使用 access() 方法。但正如我在「配置端点级授权：限制访问」和「配置端点级授权：应用访问限制」讲到非响应式应用配置时所说的，只有在无法通过其他方式实现配置时才使用 access()
...
在本节中，我们讨论如何在响应式应用的各个层面应用授权规则。对于非响应式应用，我们使用方法级安全性；在「在方法级别实现授权」和「在方法级别实现过滤」中，你已经学过了在方法级应用授权规则的不同方式。能够在端点层以外的层面应用授权规则，可以为你提供更大的灵活性，并让你能够对非
...
注解写在定义端点的方法上。不过，你也可以像「在方法级别实现授权」和「在方法级别实现过滤」中讨论的那样，在响应式应用中的其他组件方法上使用该注解。清单17.10展示了控制器类的定义。请注意，我们使用了
@PreAuthorize，就像你在「在方法级别实现授权」中学习的一样。通过 SpEL 表达式，我们声明只有管理员才能调用被注解的方法。
...
在后台，这项功能的实现与非响应式应用完全一致。在「在方法级别实现授权」和「在方法级别实现过滤」中，你已经学过，切面会拦截对方法的调用并执行授权。如果调用不满足预设的预授权规则，切面就不会将调用委托给该方法（见图17.9）。
...
方法，并传入一个合法的公钥作为参数。你可以使用我们在「OAuth 2和OpenID Connect是什么」中讨论过的任意一种方式，当时我们已经详细分析了资源服务器验证访问令牌的多种方案。
