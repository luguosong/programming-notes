...
「用户管理」和「密码管理」介绍了在认证流程中参与的一些组件。我们讨论了 UserDetails 以及如何定义原型来描述 Spring Security
...
AuthenticationProvider 的组件是 AuthenticationManager，它从 HTTP 过滤器层接收请求，这部分内容在「一个Web应用的安全性始于过滤器」已经讨论过。在本章中，我们将深入了解认证流程，其结果只有两种可能：
...
为了帮助你回忆相关角色及其之间的关系，图6.1展示了我们在「Spring Security 入门」已经见过的那张示意图。
...
本章将介绍认证流程中剩余的部分（见图6.1中阴影框部分）。接下来的「配置端点级授权：限制访问」和「配置端点级授权：应用访问限制」，你将学习授权的相关内容，也就是在HTTP请求中紧随认证之后的处理流程。首先，我们需要讨论如何实现AuthenticationProvider接口。你需要了解Spring
...
方法中接收到的参数类型。如果我们没有在认证过滤器层面做任何自定义（正如「一个Web应用的安全性始于过滤器」所讨论的），那么UsernamePasswordAuthenticationToken类就定义了这个类型。该类是Authentication接口的一个实现，代表了一个包含用户名和密码的标准认证请求。
...
本节将讨论SecurityContext。我们会分析其工作原理、数据访问方式，以及应用在不同线程相关场景下如何管理SecurityContext。完成本节学习后，你将掌握在各种情况下配置SecurityContext的方法。这样，你就能在「配置端点级授权：限制访问」和「配置端点级授权：应用访问限制」配置授权时，灵活运用SecurityContext中存储的已认证用户信息。
...
!!! note
	...
	这里我们讨论的是传统的 Servlet 应用，每个请求都对应一个线程。这种架构仅适用于每个请求分配独立线程的传统 Servlet 应用，不适用于响应式应用。关于响应式架构的安全性，我们将在「在响应式应用中实现安全性」进行详细探讨。
...
你已经知道，HTTP Basic 是默认的认证方式，我们在「用户管理」的多个示例中已经见识过它的工作原理。本节将进一步详细介绍这种认证方式的配置方法。
...
!!! note
	...
	我把这种方法与一个小型 Web 应用联系在一起，因为这样我们可以使用服务端会话来管理SecurityContext。但对于需要横向扩展的大型应用来说，使用服务端会话来管理SecurityContext并不可取。在第12到「实现OAuth2资源服务器」中，我们在讲解 OAuth 2 时会更详细地讨论这些内容。
...
只要你没有注册自己的 UserDetailsService，就可以使用默认提供的账号登录。正如我们在「Spring Security 入门」中了解到的，默认用户名是
