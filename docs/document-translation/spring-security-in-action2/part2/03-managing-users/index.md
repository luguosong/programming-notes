...
本章将深入讲解你在「Spring Security 入门」第一个示例中遇到的一个核心角色——`UserDetailsService`。除了UserDetailsService，我们还会讨论以下接口（契约）：
...
在「Spring Security 入门」中，你已经了解了UserDetailsService和PasswordEncoder在认证流程中的作用。但我们只讨论了如何使用你自定义的实例来替换Spring
...
如何设定一些默认配置，从而决定新应用最初的运行方式。你还学会了如何通过多种常见方式来覆盖这些默认设置，这些方式在实际应用中经常会用到。不过，我们之前只是简单介绍了一下这些内容，让你对接下来的学习有个大致的了解。在本章以及「密码管理」和「一个Web应用的安全性始于过滤器」中，我们将更深入地探讨这些接口，介绍不同的实现方式，并说明它们在真实项目中的应用场景。
...
这两个组件直接处理用户信息及其凭证。关于 PasswordEncoder 的详细内容，我们将在「密码管理」中进行讨论。
...
Spring Security 中，用户拥有一组权限，也就是用户被允许执行的操作。在「配置端点级授权：限制访问」到「在方法级别实现过滤」讨论授权时，我们会大量涉及这些权限。但目前，Spring
...
!!! note
	...
	正如你将在「实现身份认证」中了解到的，Spring Security 使用“权限”（authorities）来指代细粒度的权限或角色（角色是权限的集合）。为了让你阅读起来更加轻松，本书将细粒度的权限称为“权限”（authorities）。
...
正如你在第3.2.1节中看到的，UserDetails接口的定义中，将授予用户的操作称为“权限”（authorities）。在「配置端点级授权：限制访问」到「在方法级别实现过滤」中，我们会基于这些用户权限编写授权配置。因此，了解如何定义权限非常重要。
...
!!! note
	...
	请注意，这里的密码编码器是以 Function<String, String> 的形式提供的，而不是 Spring Security 提供的 PasswordEncoder 接口。这个函数的唯一职责就是对给定编码格式的密码进行转换。在下一节中，我们将详细讨论 Spring Security 的 PasswordEncoder 合约（我们在「Spring Security 入门」已经用过），并会在「密码管理」进一步深入探讨 PasswordEncoder 的相关内容。
...
是如何管理用户的呢？在进行凭证比对时，这些用户信息是从哪里获取的？又该如何添加新用户或修改已有用户？在「Spring Security 入门」中，你已经了解到，框架为用户管理专门定义了一个组件，认证流程会将用户管理的相关操作委托给
...
在下一个示例中，我们将编写一个
UserDetailsService，它包含一个内存中的用户列表。在「Spring Security 入门」中，你已经使用过一个提供的实现——InMemoryUserDetailsManager，它实现了相同的功能。由于你已经熟悉了这个实现的工作方式，这里我选择实现一个类似的功能，但这次我们自己来实现。当我们创建
...
我们在「Spring Security 入门」中使用的 InMemoryUserDetailsManager 实际上是一个 UserDetailsManager。当时我们只关注了它作为 UserDetailsService
