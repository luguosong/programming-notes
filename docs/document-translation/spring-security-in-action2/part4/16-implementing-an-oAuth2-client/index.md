...
HTTP Basic 或 API Key（「实现身份认证」）等更简便的认证方式，但从设计和安全角度来看，使用 OAuth 2 的客户端凭证授权类型是更理想的选择。

还记得 OAuth 2 的各个参与者（图 16.1）吗？我们在「实现OAuth 2授权服务器」讨论了授权服务器，在「实现OAuth2资源服务器」讨论了资源服务器。本章将专注于客户端。我们将探讨如何使用
...
如果你需要快速复习一下如何使用 Spring Boot 构建 Web 应用，我另一本书《Spring Start Here》（Manning，2020）的「配置端点级授权：限制访问」和「配置端点级授权：应用访问限制」可以帮助你快速回忆这些技能。下面的代码片段展示了我们这个示例 Web 应用中最基础的控制器，它目前只包含一个主页：
...
13 至「实现OAuth2资源服务器」的讲解，并在这些章节中通过 cURL 多次进行了演示。
...
我猜你在想，是不是还得像「OAuth 2和OpenID Connect是什么」到「实现OAuth2资源服务器」里学的那样填写授权 URL、令牌 URL、客户端 ID、客户端密钥等等？没错，这些信息仍然必需。幸运的是，Spring
...
LinkedIn、Twitter、Yahoo 等。你也可以使用在「实现OAuth 2授权服务器」中学习过的自定义授权服务器。

你可以使用任意提供商（包括你自己搭建的）来配置 OAuth 2 登录。本节将利用「实现OAuth 2授权服务器」中搭建的授权服务器，展示如何配置自定义的
OAuth 2 登录。为了方便学习并保持示例独立，我已将「实现OAuth 2授权服务器」中讨论的 ssia-ch14-ex1 项目内容复制到本章的一个新项目中，命名为
ssia-ch16-ex1-as。
...
Security 已经掌握了通用提供者所需的所有细节。但如果要使用不同的提供者，就需要配置几项内容。Spring Security 需要知道以下内容（如「OAuth 2和OpenID Connect是什么」和「实现OAuth 2授权服务器」所述）：
...
由于「实现OAuth 2授权服务器」中构建的授权服务器已正确实现 OpenID Connect 协议，因此我们可以依赖发行者 URI。下面的代码片段展示了如何配置发行者
...
还记得我们从「Spring Security 入门」开始讨论的 Spring Security 身份验证设计（如图16.7所示）吗？成功验证的最后一步总是将认证信息添加到安全上下文中，使用
oauth2Login() 也不例外。
...
- 可以使用「在方法级别实现授权」和「在方法级别实现过滤」中讨论的前置/后置注解。
...
16.8 回顾了我们在「OAuth 2和OpenID Connect是什么」讨论的客户端凭证授权类型。
...
我们将沿用「实现OAuth 2授权服务器」中构建的授权服务器，本章对应的项目是
ssia-ch16-ex1-as。请记得先在授权服务器中添加一个客户端注册，支持使用客户端凭证授权类型。你可以修改「实现OAuth 2授权服务器」中已经配置过的那个（如下节所示），也可以新增一个满足该要求的客户端注册。
