...
本章探讨如何在 OAuth 2 体系中保护后端应用。按照 OAuth 2 的术语，资源服务器其实就是一个后端服务。「实现OAuth 2授权服务器」我们学习了如何用
Spring Security 实现授权服务器的职责，现在该讨论如何使用授权服务器生成的令牌了。

在实际场景中，你可能会也可能不会像「实现OAuth 2授权服务器」那样自建授权服务器；很多团队更愿意采用第三方实现。可选方案很多，从 Keycloak
这样的开源项目到 Okta、Cognito、Azure AD 等企业级产品一应俱全——本书第一版的「测试安全配置」就提供了 Keycloak 的示例。
...
在这个示例中，你需要使用一个授权服务器。可以使用我们在「实现OAuth 2授权服务器」、项目 ssia-ch14-ex1 中创建的那个。
...
请记住在「实现OAuth 2授权服务器」中提到，OpenID Connect 授权服务器会提供一个 URL，用于获取其配置（包括授权、令牌、公钥集等 URL）。下面的示例展示了所谓的
well-known URL：
...
现在该启动刚刚创建的资源服务器应用了。请确保授权服务器依然在运行。接下来需要利用「实现OAuth 2授权服务器」学到的技能来生成访问令牌。我们再回顾一下授权码授权流程的步骤（不过请记住，你也可以使用任何其他授权模式获取令牌——对资源服务器而言，只要你持有访问令牌，获取方式并不重要）。
...
要发送自省请求，资源服务器需要客户端凭证进行身份验证，就像其他任何客户端一样。在这个示例中，我将修改我们在「实现OAuth 2授权服务器」讨论不透明令牌时创建的
ssia-ch14-ex4 项目：
...
请记得也要加入授权配置。下面的代码片段展示了你在「配置端点级授权：限制访问」和「配置端点级授权：应用访问限制」中学到的标准做法，让所有端点都必须进行请求认证：
...
端点发送请求。为简化示例，我采用了客户端凭证授权类型，但您也可以使用「实现OAuth 2授权服务器」所介绍的任意一种授权类型来获取访问令牌。请记住，无论通过何种方式获取访问令牌，资源服务器的配置都是相同的。
