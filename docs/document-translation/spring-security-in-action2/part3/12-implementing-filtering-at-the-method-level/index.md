...
在「在方法级别实现授权」中，你已经学习了如何使用全局方法安全来应用授权规则。我们通过示例演示了 @PreAuthorize 和 @PostAuthorize
...
就像我们在「在方法级别实现授权」讨论的调用授权一样，Spring Security 也通过切面实现过滤。切面可以拦截特定方法的调用，并为其附加额外指令。对于预过滤而言，切面会拦截使用
...
与「在方法级别实现授权」中讨论的 @PreAuthorize 和 @PostAuthorize 注解类似，你可以在 @PreFilter 注解的值中设置授权规则。在这些以 SpEL
...
@GetMapping 来处理一个表示变更的操作，这种做法并不标准。但请理解，我这样做是为了避免在示例中处理 CSRF 保护，从而让你能更专注于当前的主题。「配置CSRF防护」里我们已经介绍过 CSRF 保护。
...
要应用后置过滤（postfiltering），需要使用 @PostFilter 注解。@PostFilter 的使用方式与我们在「在方法级别实现授权」及本章中涉及的其他前置/后置注解类似。你需要将授权规则作为
