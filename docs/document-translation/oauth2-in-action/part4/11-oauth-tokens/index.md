...
无论 OAuth 协议里有多少重定向、流程和组件，归根结底讲的都是令牌。回想「什么是 OAuth 2.0，以及你为什么需要关注？」的云打印示例：为了让照片存储服务确认打印机具备访问照片的权限，打印机服务需要拿出某种东西来证明授权。打印机交给存储服务的这个东西，我们称之为访问令牌（access
...
尽管 OAuth 把大量精力都放在令牌（token）上，但它完全不会对令牌的内容是什么做任何主张或说明。正如我们在「OAuth 之舞」和「构建一个简单的OAuth客户端」讨论过的，在
OAuth
...
让我们为授权服务器加入 JWT 支持。打开 ch-11-ex-1，编辑 authorizationServer.js 文件。在「构建一个简单的 OAuth 授权服务器」中，我们实现了一个服务器，用来签发非结构化的随机令牌。这里我们将改造该服务器，使其生成不带签名、但符合 JWT
...
!!! note "我得把密钥到处拷贝一遍吗？  "
	...
	一种常见的做法（OpenID Connect 协议就采用了这种方式，我们会在「使用 OAuth 2.0 进行用户认证」介绍）是让授权服务器在一个固定的 URL 上发布它的公钥。通常会以 JWK Set 的形式提供，其中可以包含多个密钥，大致如下所示。
...
token）来完成认证——「基于 OAuth 2.0 的协议与配置文件」讨论的 UMA 协议就是这么做的。
...
内省响应还可以包含令牌的 scope，形式与最初的 OAuth 请求一致，为以空格分隔的 scope 字符串列表。正如我们在「构建一个简单的 OAuth 受保护资源」看到的，令牌的
...
getProtectedResource 函数，用来对应「构建一个简单的 OAuth 授权服务器」里创建的 getClient 函数。
...
protectedResource.js 并进行编辑。首先，为受保护资源配置它的 ID 和密钥（secret），就像我们在「构建一个简单的 OAuth 授权服务器」为客户端做的那样。
