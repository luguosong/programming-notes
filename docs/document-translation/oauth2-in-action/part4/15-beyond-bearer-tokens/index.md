...
Token 都是 Bearer Token。正如我们在「常见的 OAuth 令牌漏洞」所讨论的，Bearer Token 可以被任何携带（bear）它的人拿去访问受保护资源。这是一种在许多系统中有意为之的设计选择，并且在
...
持有者令牌非常好用，因为客户端无需额外处理或理解即可使用。回想「什么是 OAuth 2.0，以及你为什么需要关注？」和「OAuth 之舞」的讨论，OAuth 2.0
...
Web Key（JWK）格式，这是我们在「OAuth 令牌」讨论过的 JSON Object Signing and Encryption（JOSE）规范套件的一部分。JWK
...
这个 JWK 是一对 RSA 密钥（我们在「OAuth 令牌」已经见过），客户端可以在下一步用它来对请求进行签名。由于这是 RSA
密钥，授权服务器在生成密钥对后只需要保存公钥部分即可，从而避免授权服务器遭到攻击时泄露私钥等关键材料。

在我们的示例中，访问令牌本身是一个随机字符串；当然，它也完全可以像「OAuth 令牌」所述那样采用 JSON Web
...
introspection，「OAuth 令牌」讨论）之类的服务来查询。这些流程与持有者令牌基本一致，只有一个关键差异。
...
PoP。这个练习中，服务器端还需要做最后一件事：在令牌自省（introspection）响应里返回访问令牌的密钥，因为接下来我们会通过令牌自省来查询令牌的详细信息（更多内容见「OAuth 令牌」）。请在自省端点添加下面这一行：
...
现在我们已经拿到了这个请求体，接下来就按「OAuth 令牌」里的同样步骤，创建一个用 JWS 签名的对象。我们会用之前保存的、与访问令牌关联的密钥来对我们的
payload 进行签名。
...
这在机制上与授权服务器在「OAuth 令牌」创建签名令牌时所做的事情类似，但你会发现我们这里并不是在创建令牌。事实上，我们是在签名对象里把令牌包含进去。另外，别忘了我们现在是在客户端内部操作，而客户端并不会签发令牌。我们要做的是生成一个签名，受保护资源可以验证该签名，从而证明我们这个客户端确实持有正确的密钥。正如我们将在下一节看到的，这并不能说明被包含的令牌具体能做什么，甚至也无法说明它是否有效。
...
现在我们需要像「OAuth 令牌」那样解析 JWS 结构。我们按句点（.）分割字符串，并对 header 和 payload 进行解码。拿到对象形式的 payload
...
一样，这里也有几种实现方式可选，例如查数据库，或者将 at 的值按 JWT 解析。本文练习中，我们会像「动态客户端注册」那样，通过令牌自省（token
