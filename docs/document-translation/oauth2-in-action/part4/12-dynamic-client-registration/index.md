...
OAuth 客户端的软件应用。在交互式 OAuth 流程（例如我们在「构建一个简单的OAuth客户端」至「构建一个简单的 OAuth 授权服务器」实现的授权码模式）中，客户端 ID
...
又或者，同一款客户端软件有许多实例，而每个实例都需要与同一个授权服务器通信呢？正如「真实世界中的 OAuth 2.0」所提到的，这在原生应用中非常常见，每个客户端实例都需要一个客户端标识符来与授权服务器交互。我们同样可以说：所有实例都使用同一个标识符，在某些情况下确实可行。但客户端密钥（client
secret）该如何处理？我们在「常见客户端漏洞」已经知道，不能把同一个密钥复制到各处，因为那样它就不再“秘密”了。[^1]
...
authorizationServer.js 来完成本节练习。我们的授权服务器将继续沿用「构建一个简单的 OAuth 授权服务器」中用于客户端功能的同一个内存数组，这意味着每次服务器重启后，这份存储都会被重置。相比之下，在生产环境中通常会使用数据库或其他更可靠的存储机制。
...
我们不再像「构建一个简单的OAuth客户端」那样手动填写，而是改用动态注册协议。和之前一样，这是一种基于内存的存储方案：每次客户端软件重启都会被重置；在生产环境中，通常会用数据库或其他存储机制来承担这一角色。
...
OAuth 动态客户端注册，在此基础上又扩展了少量自身的元数据项，这些项是 OpenID Connect 协议特有的，我们将在「使用 OAuth 2.0 进行用户认证」介绍。我们在表
...
URI。正如你在「常见客户端漏洞」和「常见的授权服务器漏洞」看到的，如果授权服务器不够谨慎，这会引发一系列漏洞。
