...
对于使用授权码流程的应用来说，一种可行的缓解措施是使用第 10 章讨论的 Proof Key for Code Exchange（PKCE）。PKCE
...
解决该问题的另一种思路是使用第 12
章深入讨论的动态客户端注册。通过动态客户端注册，某个客户端软件实例可以在运行时自行完成注册。这实际上把原本需要在配置阶段保存的秘密转换成了运行时秘密，从而为那些原本无法使用相关能力的客户端提供更高的安全性与功能性。
