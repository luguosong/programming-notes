...
app.get('/fetch_resource', function(req, res) {
	...
	if (resource.statusCode >= 200 && resource.statusCode < 300) {
		...
		/*
		 * 不要像这里这样总是直接返回错误；如果我们有刷新令牌，就刷新访问令牌。
		 */
...
var refreshAccessToken = function(req, res) {
	...
	var form_data = qs.stringify({
		...
		refresh_token: refresh_token //请求体中传递刷新令牌
	});
	var headers = {
		'Content-Type': 'application/x-www-form-urlencoded',
		'Authorization': 'Basic ' + encodeClientCredentials(client.client_id, client.client_secret)
	...
	if (tokRes.statusCode >= 200 && tokRes.statusCode < 300) {
		...
		console.log('Got access token: %s', access_token);
		if (body.refresh_token) {
			// 丢弃之前一直保存的旧 refresh token，并立即开始使用新的。
			refresh_token = body.refresh_token;
			console.log('Got refresh token: %s', refresh_token);
		}
		scope = body.scope;
		...
		// 让客户端再尝试获取一次资源。由于客户端的操作是由 URL 触发的，
		// 我们可以重定向回获取资源的 URL，重新启动整个流程。在线上环境中，
		// 实际实现通常会采用更精细、更完善的触发机制。
		res.redirect('/fetch_resource');
		return;
	} else {
		console.log('没有刷新令牌，需要提示用户获取新的访问令牌');
		refresh_token = null;
