...
// 通过父目录 + 子路径创建
File file2 = new File("D:/test", "hello.txt");
...
``` java
// ✅ 推荐：每次读取一批字节，减少磁盘 IO 次数
try (FileInputStream fis = new FileInputStream("test.txt")) {
    byte[] buffer = new byte[1024]; // 1KB 缓冲区
    int bytesRead;
    while ((bytesRead = fis.read(buffer)) != -1) {
        // 注意：最后一次可能读不满，要用 bytesRead 控制长度
...
``` java
try (FileOutputStream fos = new FileOutputStream("output.txt")) {
    byte[] data = {72, 101, 108, 108, 111}; // "Hello" 的 ASCII
    fos.write(data);

    // 只写数组的一部分：从 offset 开始，写 length 个字节
    fos.write(data, 0, 3); // 只写 "Hel"
}
```
...
``` java
// 从字节数组读取
byte[] source = "Hello from memory".getBytes();
...
// 写入字节数组——ByteArrayOutputStream 内部维护一个可扩容的 byte[]
try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
    baos.write("Hello ".getBytes());
    baos.write("World".getBytes());
    byte[] result = baos.toByteArray(); // 获取最终的字节数组
...
``` java
// 用 GBK 编码读取文件（解决 GBK 文件的乱码问题）
try (InputStreamReader isr = new InputStreamReader(
        new FileInputStream("gbk-file.txt"), "GBK")) {
    int ch;
    while ((ch = isr.read()) != -1) {
...
try (OutputStreamWriter osw = new OutputStreamWriter(
        new FileOutputStream("utf8-file.txt"), StandardCharsets.UTF_8)) {
    osw.write("你好，世界！");
}
```
...
``` java
// FileReader 等价于 new InputStreamReader(new FileInputStream(file), 默认编码)
...
``` java
// 带缓冲的文件拷贝——性能远高于裸 FileInputStream
try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream("big-file.zip"));
...
try (BufferedReader br = new BufferedReader(new FileReader("data.txt"))) {
    br.mark(100); // 标记当前位置，参数为预读上限（字符数）
    String first = br.readLine();
    br.reset(); // 回退到 mark 位置
    String again = br.readLine(); // 再次读取同一行
}
```
...
``` java
// 被序列化的类必须实现 Serializable（标记接口，无需实现任何方法）
...
try (ObjectInputStream ois = new ObjectInputStream(
    ...
    System.out.println(user.getPassword()); // null（transient 字段不被序列化）
}
```
...
try (GZIPOutputStream gos = new GZIPOutputStream(
    ...
    gos.write("需要压缩的大量文本数据...".getBytes(StandardCharsets.UTF_8));
}
...
String port = props.getProperty("db.port", "3306"); // 提供默认值
```
...
``` java
// 自动在 classpath 中查找 config.properties（注意不带后缀）
