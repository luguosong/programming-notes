...
SQL 标准定义了一系列高级数据类型，JDBC 为这些类型提供了对应的 Java 映射支持。主要包括：
...
实际开发中，这些类型多由 ORM 框架（如 MyBatis、JPA）处理，直接使用 JDBC 操作的场景较少。但在需要利用数据库原生能力（如 XML 索引查询、数组包含判断）时，了解 JDBC 高级类型的操作方式仍然有价值。
