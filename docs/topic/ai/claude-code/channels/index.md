...
=== "Telegram"
    ...
    **第 1 步**：在 Telegram 中打开 [BotFather](https://t.me/BotFather)，发送 `/newbot` 创建机器人，复制返回的令牌。
    ...
    **第 4 步**：配对账户。向机器人发送任意消息，机器人会回复配对代码。在 Claude Code 中运行：
...
=== "Discord"
    ...
    **第 1 步**：在 [Discord 开发者门户](https://discord.com/developers/applications) 创建应用，在**机器人**部分创建并复制令牌。在**特权网关意图**中启用**消息内容意图**。

    **第 2 步**：在 **OAuth2 > URL 生成器** 中选择 `bot` 范围，勾选以下权限：
    ...
    **第 5 步**：配对账户。向机器人发送私信获取配对代码，然后在 Claude Code 中运行：
...
=== "iMessage"
    ...
    **第 1 步**：授予完全磁盘访问权限。首次读取 `~/Library/Messages/chat.db` 时，macOS 会弹出权限提示，点击**允许**。如果没有弹出，在**系统设置 > 隐私和安全 > 完全磁盘访问**中手动添加终端。
    ...
    **第 3 步**：给自己发短信即可开始使用（自聊天绕过访问控制）。如果需要允许其他联系人，添加他们的句柄：
...
**第 2 步**：使用 `--channels` 标志启动 Claude Code：
...
**第 3 步**：打开 [http://localhost:8787](http://localhost:8787)，输入消息：
