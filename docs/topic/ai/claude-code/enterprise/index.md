...
企业管理本质上是一连串决策，本文按此顺序组织：

1. **选哪个 API 提供商**（决定计费和合规体系）
2. **策略怎么到达开发者设备**（云端推送还是 MDM）
3. **要强制执行什么规则**（权限、沙箱、插件限制）
4. **怎么追踪用量和成本**
...
??? tip "各机制的安全性差异"
    ...
    - **plist / HKLM 注册表**：需要管理员权限才能写入，抗篡改能力强，适合作为主要执行通道（HKLM 即 HKEY_LOCAL_MACHINE，写入该注册表项需要管理员权限，普通用户无法修改）
    - **文件策略**：同样需要管理员权限写入系统目录，通用性最好（支持所有平台）
    - **HKCU 注册表**：无需提升权限即可写入（HKCU 即 HKEY_CURRENT_USER，当前登录用户可直接修改），仅作为便利默认值，不应作为强制执行通道
...
| [沙箱隔离](../configuration/settings-permissions/index.md) | 操作系统级文件系统和网络隔离 | `sandbox.enabled`、`sandbox.network.allowedDomains`、`sandbox.network.deniedDomains` |
...
v2.1.128 对 `OTEL_*` 环境变量做了隔离：子进程（Bash、hooks、MCP、LSP）不再继承这些变量。之前通过 Bash 工具运行的 OTEL 仪表化应用可能误用 CLI 的 OTLP 端点，导致遥测数据混入 Claude Code 的指标中。隔离后各进程的遥测互不干扰。
...
如果需要更强的隔离，可以将 Claude Code 运行在开发容器（DevContainer）中——容器化环境提供操作系统级隔离，同时保证团队开发环境一致性。
