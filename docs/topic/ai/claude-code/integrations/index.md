...
| **模型固定** | 推理配置文件 ID（用于固定模型版本） | 区域端点 | 自动 | 代理配置 |
...
> Bedrock 的「推理配置文件 ID」用于将 API 请求固定到特定模型版本，避免 Bedrock 平台自动更新模型带来的行为变化。你可以在 AWS Bedrock 控制台的 **Inference Profiles** 页面找到它。
...
Claude Code 的配置体系通过 `.claude/` 目录管理，分为**项目级**和**全局级**两个作用域。理解这套目录结构有助于你：

- **团队协作**：将 `CLAUDE.md` 和 `.claude/rules/` 纳入版本控制，确保所有成员的 Claude Code 行为一致
- **个性化配置**：通过全局级 `~/.claude/` 设置个人偏好（如默认权限、常用 MCP 服务器），不影响团队共享配置
- **问题排查**：当 Claude Code 行为异常时，用诊断命令快速定位是哪一层配置在生效
