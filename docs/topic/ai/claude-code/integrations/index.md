...
Claude Code 提供了交互式配置向导来简化接入流程。运行 `/setup-bedrock` 或 `/setup-vertex` 即可一步步完成配置（2.1.111 改进：当 `CLAUDE_CONFIG_DIR` 自定义时显示实际的 `settings.json` 路径，重新运行时从已有 pin 中获取候选模型）。
