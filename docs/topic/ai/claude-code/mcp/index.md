...
- 📋 MCP 资源引用（`@` 语法）、提示命令（`/mcp__` 语法）
- ☁️ Claude.ai MCP 同步与插件 MCP 服务器来源
- 🙋 Elicitation：服务器请求用户输入的交互机制
...
MCP 就像是**给实习生配了一套专业工具箱**——里面有数据库客户端、消息工具、项目管理工具等。通过标准化的协议（MCP），实习生可以查数据库、读工单、看监控、发消息——而这一切只需要你用自然语言下一句指令。大多数工具箱就放在实习生手边（本地运行），少数需要连到远程服务（云端 MCP）。

在这个比喻中：

- **MCP 服务器**（MCP Server）= 工具箱中的某个专业工具（比如 GitHub 客户端、数据库客户端）
- **MCP 客户端**（MCP Client）= Claude Code 自身，它负责与这些工具通信
- **传输协议** = 工具和 Claude Code 之间的通信方式（本地进程通信或网络 HTTP）
...
在 Claude Code 会话中，使用 `/mcp` 查看所有服务器的连接状态、进行 OAuth 认证或清除身份验证。从 v2.1.128 起，`/mcp` 还会显示每个已连接服务器的工具数量，并特别标记连接成功但工具数为 0 的服务器——这能帮你快速发现配置错误或启动失败的服务器。
...
`.mcp.json` 支持**环境变量展开**——Claude Code 会在加载配置时自动替换这些变量占位符，方便团队共享配置但各自设置敏感信息：
...
``` json title="内联 headersHelper 命令"
{
  "mcpServers": {
    "internal-api": {
      "type": "http",
      "url": "https://mcp.internal.example.com",
      "headersHelper": "echo '{\"Authorization\": \"Bearer '\"$(cat ~/.config/internal-api-token)\"'\"}'"
    }
  }
}
```

⚠️ 内联命令中引号嵌套容易出错，复杂场景建议改为脚本文件引用方式（如上面的 `/opt/bin/get-mcp-auth-headers.sh`），脚本中可以安全地调用 `aws ssm get-parameter`、`vault read` 等命令获取 token。
...
⚠️ `workspace` 是 MCP 服务器的保留名称（v2.1.128 新增）。如果你已经配置了名为 `workspace` 的服务器，它会被跳过并给出警告——需要改用其他名称。
...
从 v2.1.128 起，重连 MCP 服务器时不再向会话注入完整工具列表——重宣告的工具按服务器前缀汇总显示，减少重连时的上下文噪音。
...
除了手动配置，MCP 服务器还有两个来源：从 Claude.ai 同步和通过插件提供。
...
Claude Code 也可以**反过来作为 MCP 服务器**，供其他应用连接。典型场景是：当你在 Claude Desktop 中想调用 Claude Code 的能力（如代码编辑、文件搜索、终端操作等），可以通过 MCP 协议让 Claude Desktop 使用 Claude Code 的工具集。
