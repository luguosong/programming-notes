...
| 场景 | 推荐方案 |
|------|---------|
| 在手机上继续本地会话 | Remote Control（远程控制本地正在运行的 Claude Code 会话） |
| 从 Telegram/Discord/Slack 推送事件到会话 | Channels（将外部消息平台的提示词自动转发到 Claude Code） |
...
| 构建自定义 Agent 工作流 | Agent SDK（用代码编排多步 Agent 调用流程） |
...
4. **进阶扩展** — 根据需要探索：上下文工程（精准投喂上下文）、多平台支持（VS Code/JetBrains 等）、扩展体系（Skills、Hooks、MCP、Sub-agents、Plugins、Channels）
5. **自动化与集成** — 将 Claude Code 接入 CI/CD、Slack 等工作流
6. **最佳实践与故障排除** — 常见反模式、验证闭环、排错方法论
