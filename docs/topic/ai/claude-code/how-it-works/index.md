...
- 🎨 终端界面进阶功能与快捷键（TUI 模式、思考力度控制、Vim 可视模式）
...
Claude Code 本身就是套在 Claude 模型外面的**"代理线束"（Agentic Harness）**——就像给一个只会说话的大脑装上了手脚和工具箱。它提供工具集、上下文管理和执行环境，把一个语言模型变成一个能真正干活的编码代理。
...
📝 v2.1.126 移除了 `Read` 工具的恶意软件评估提示。这个提示原本是为了安全防护，但在实际使用中可能导致旧模型误判正常文件为恶意内容。移除后 `Read` 工具的行为更加可预测，减少不必要的干扰。
...
检查点（Checkpointing）和上下文压缩（Compaction）容易混淆，但它们解决的是完全不同的问题：

| 维度 | 检查点（Checkpointing） | 上下文压缩（Compaction） |
|------|----------------------|------------------------|
| **解决什么** | 代码版本回退——"改错了，退回去" | 上下文空间管理——"对话太长，装不下了" |
| **操作对象** | 磁盘上的文件状态 | 会话中的对话历史 |
| **触发方式** | `Esc` + `Esc`、`/rewind`、`/undo` | `/compact` 命令或自动触发 |
| **回退效果** | 文件恢复到之前的状态 | 对话历史被压缩为摘要，文件不受影响 |
| **数据保留** | 保留完整的文件快照 | 压缩时早期详细指令可能丢失 |
| **与 Git 的关系** | 类似本地撤销，不是 Git 的替代品 | 无关，压缩只影响对话上下文 |

💡 简单记忆：**检查点管代码，压缩管对话**。检查点让你在文件改动上大胆试错，压缩让长会话继续跑下去。两者配合使用——用检查点回退不满意的代码改动，用压缩释放上下文空间继续工作。

⚠️ 影响远程系统（数据库、API、部署）的操作无法被检查点覆盖，所以 Claude 在执行这类命令前会主动征求你的同意。这也是为什么权限控制如此重要。

## 🎨 终端界面与快捷键

Claude Code 的终端界面（TUI）提供了多种交互方式和快捷键来提升操作效率。基础快捷键（如 `Ctrl+C` 中断、`Ctrl+D` 退出、`Ctrl+R` 搜索历史等）已在「快速入门」中介绍，这里聚焦进阶功能。
