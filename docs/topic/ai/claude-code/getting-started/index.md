...
- 🎯 安装与配置（安装方式、系统要求、认证流程）
- 💬 交互模式操作（REPL 对话、斜杠命令、快捷键、多行输入）
- ⌨️ CLI 命令速查（交互模式与非交互模式的常用写法）
- 🔍 隐藏技巧（`/simplify`、`/insight`、双击 ESC 等）
...
在项目目录下输入 `claude`，你就进入了**交互模式**（也叫 REPL 模式）。这个名字来源于编程中的一个经典概念：**Read-Eval-Print Loop**（读取-求值-打印-循环）——就像对讲机对话：你说一句，对方回一句，来回循环。你输入一句话，Claude 思考后回复，如此往复。
...
| `/usage` | 综合用量面板，包含费用和统计信息。`/cost` 和 `/stats` 仍可使用作为快捷入口打开对应标签页（v2.1.118 合并） |
...
Claude 会检查当前对话中修改过的文件，找出重复代码、低效实现和潜在问题，然后直接给出修复建议。你也可以指定关注点：
...
- 一次较长的调试会话后，提炼出发现的编码规范和注意事项
- 完成新功能开发后，把项目结构和架构决策的共识写入 `CLAUDE.md`
- 多轮对话中反复纠正 Claude 的同一类错误时，用 `/insight` 把规则固化下来
