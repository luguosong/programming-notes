...
核心原则只有一条：**先诊断，再修复**——就像看病先验血再开药，盲目改配置往往越改越乱。类比汽车维修：你不会一听到异响就直接换发动机，而是先打开引擎盖找到具体哪个零件出了问题。`/doctor` 就是你最好的起点——它能一次性检查安装状态、配置有效性、MCP 服务器连接和上下文使用情况，省去你逐项排查的时间。
...
- **MCP 服务器无响应**：某个 MCP 服务器进程卡住，导致 Claude Code 等待工具调用超时。运行 `/mcp` 查看各服务器状态，对异常的服务器选择"重新连接"或"禁用"
- **大文件读取**：Claude 尝试一次性读取超大文件时，内存飙升导致卡顿。建议在 `CLAUDE.md` 中引导 Claude 分块读取
- **网络代理阻塞**：企业代理对长时间连接进行超时中断。检查 `HTTPS_PROXY` 配置，或尝试设置 `API_TIMEOUT_MS` 增加超时时间
