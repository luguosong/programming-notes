...
- 按错误类型（认证、服务端、限流、网络、请求内容）快速定位解决方案
- 理解自动重试机制和 `CLAUDE_CODE_MAX_RETRIES` 的作用
- 掌握响应质量异常和其他问题的排查思路

> Claude Code 在展示错误前会自动重试瞬时故障（指数退避，最多 10 次）。指数退避就像拨打电话占线后逐渐增加等待时间再重试——第一次等 1 秒，第二次 2 秒，第三次 4 秒，以此类推。当看到下方错误时，说明重试已耗尽。可通过 `CLAUDE_CODE_MAX_RETRIES` 调整重试次数，通过 `API_TIMEOUT_MS`（默认 600000ms）调整请求超时。
