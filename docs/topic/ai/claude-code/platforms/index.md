...
| **无头模式** | 编程调用 | 结构化输出、Agent SDK 集成 | CI/CD、自动化工作流 |
...
VS Code 扩展将 Claude Code 嵌入编辑器侧边栏，让你**不用离开 IDE 就能和 Claude 对话**。
...
Claude Code Desktop 是一个**独立的图形化应用**，为不习惯终端的用户提供了更友好的入口。
...
Web 版的核心是 **Teleport 技术**：你的代码会自动上传到 Anthropic 的云端虚拟机，Claude Code 在云端运行，修改后的代码再同步回你的 GitHub 仓库。
...
Chrome 扩展是一个**比较特殊的存在** —— 它不是为了在浏览器里写代码，而是为了让 Claude Code 能够**控制和调试 Web 应用**。
