...
- 终端 CLI、VS Code 扩展、JetBrains 插件各自的特色功能
- 桌面应用和 Web 版如何让非终端用户也能用上 Claude Code
- Chrome 扩展和 DevContainer 两个特殊场景
- 无头模式（Headless）如何支撑自动化工作流
- Computer Use 的平台可用性约束和 CLI 与 Desktop 的差异
- Remote Control 在各平台的启动方式、连接手段和移动推送通知
...
graph TD
  ...
  Engine --> CU["Computer Use<br/>（macOS 研究预览）"]
  Engine --> RC["Remote Control<br/>（远程操控本地会话）"]
  Engine --> DevC["DevContainer"]
  Engine --> Headless["无头模式"]

  style Engine fill:transparent,stroke:#539bf5,color:#adbac7,stroke-width:2px
  style CU fill:transparent,stroke:#f57c00,color:#adbac7,stroke-width:1px
  style RC fill:transparent,stroke:#f57c00,color:#adbac7,stroke-width:1px
...
| **Chrome 扩展** | 浏览器控制台 | 调试 Web 应用、结构化 DOM 访问 | 前端调试场景 |
| **Computer Use** | 屏幕操控（macOS） | 操控桌面应用、截图、UI 测试 | macOS 原生应用验证 |
| **Remote Control** | 远程操控本地会话 | 手机/浏览器驱动本地 Claude Code | 离开工位时继续任务 |
...
- Chrome 扩展支持 macOS、Windows 和 Linux（Computer Use 详见下方章节，仅 macOS）
- 它本质上是一种**浏览器自动化**能力，类比 Selenium/Playwright，但由 AI 驱动

## 🖥️ Computer Use 的平台可用性

Computer Use 是 Claude Code 中一项让 Claude 直接操控你电脑界面的能力（点击、输入、截图），在「第三方集成」笔记中已经介绍了它的应用分级控制、安全机制和工作流。这里从**平台视角**补充可用性信息。

### 可用性约束

Computer Use 目前是一个 **macOS 研究预览版**，有明确的平台和使用限制：

| 条件 | 要求 |
|------|------|
| 操作系统 | **仅 macOS**（Linux 和 Windows CLI 均不支持） |
| 最低版本 | Claude Code v2.1.85 或更高 |
| 订阅计划 | **Pro 或 Max**（Team 和 Enterprise 不可用） |
| 认证方式 | 必须通过 `claude.ai` 登录（Bedrock、Vertex AI、Foundry 不支持） |
| 会话模式 | **交互式会话**（使用 `-p` 标志的非交互模式不可用） |
| 内置 MCP 服务器名称 | `computer-use`（在 `/mcp` 菜单中启用） |

如果你在 `/mcp` 中看不到 `computer-use` 服务器，逐一排查上表中的条件即可定位原因。运行 `claude --version` 检查版本，`/status` 确认订阅。

### CLI 与 Desktop 的差异

Computer Use 的底层引擎在 CLI 和 Desktop 应用之间是共享的，但 Desktop 提供了一些 CLI 尚不具备的图形化控制：

| 功能 | Desktop 应用 | CLI |
|------|-------------|-----|
| 启用方式 | Settings > General 中的开关 | `/mcp` 菜单中启用 `computer-use` |
| 应用拒绝列表 | 在设置中配置 | 尚不可用 |
| 自动取消隐藏 | 可选切换 | 始终开启 |
...
从平台运行时的角度看，Computer Use 有两个值得注意的行为：

- **锁文件机制**：Computer Use 活跃时持有机器范围的锁，同一时间只有一个 Claude Code 会话可以控制你的机器。如果另一个会话已持有锁，新尝试会失败并提示哪个会话正在使用。进程崩溃后锁会自动释放。
- **全局转义**：在任何地方按 `Esc` 会中止 Computer Use，且按键事件被系统消耗，不会被应用接收。这防止了通过屏幕内容的提示注入利用 Esc 关闭对话框。

更详细的分级控制表、安全哨兵机制和工作流示例，参见「第三方集成」笔记中的 [Computer Use 章节](../integrations/index.md)。

## 📡 Remote Control 的平台支持

Remote Control 让你在手机、平板或另一台电脑上继续操控本地运行的 Claude Code 会话。它与「Web 版」不同——Remote Control 中 Claude 始终在你**本地机器**上运行，手机和浏览器只是一个远程窗口。

### 可用性

| 条件 | 要求 |
|------|------|
| 最低版本 | Claude Code v2.1.51 或更高 |
| 订阅计划 | Pro、Max、Team、Enterprise 均可用（Team/Enterprise 需管理员在 Claude Code 管理员设置中启用） |
| 认证方式 | `claude.ai` OAuth 登录（不支持 API Key） |
| 前置操作 | 在项目目录中至少运行一次 `claude` 以接受工作区信任对话框 |

### 各平台的启动方式

Remote Control 在 CLI 和 VS Code 中有不同的启动体验：

| 平台 | 启动方式 | 特有功能 |
|------|---------|---------|
| **CLI 服务器模式** | `claude remote-control` | 支持多并发会话、按空格键显示 QR 码、`--spawn` 控制会话创建方式（`same-dir`/`worktree`/`session`） |
| **CLI 交互模式** | `claude --remote-control` 或 `claude --rc` | 本地和远程可同时使用，支持自定义会话名称 |
| **CLI 会话内** | `/remote-control` 或 `/rc` | 继承当前对话历史，显示会话 URL 和 QR 码 |
| **VS Code** | `/remote-control` 或 `/rc`（v2.1.79+） | 横幅显示连接状态，不支持名称参数和 QR 码显示 |

### 远程连接方式

一旦 Remote Control 会话处于活跃状态，你有三种方式从另一台设备连接：

- **会话 URL**：在任意浏览器中打开，直接跳转到 `claude.ai/code` 上的会话
- **QR 码**：CLI 模式下按空格键切换 QR 码显示，用 Claude 移动应用扫描即可打开（CLI 专属功能，VS Code 不支持）
- **会话列表**：打开 `claude.ai/code` 或 Claude 应用，在线的 Remote Control 会话会显示绿色计算机图标

### 移动推送通知（v2.1.110+）

当 Remote Control 处于活动状态时，Claude 可以向你的手机发送推送通知。Claude 会自主判断何时推送（通常在长时间任务完成或需要你做决定时），你也可以在提示中主动请求（例如 `notify me when the tests finish`）。

推送通知的设置步骤：

1. 下载 Claude 移动应用（iOS / Android）
2. 使用与 Claude Code 相同的账户登录
3. 接受操作系统的通知权限
4. 在 Claude Code 中运行 `/config`，启用「当 Claude 决定时推送」

### 与 Web 版的关键区别

| 维度 | Remote Control | Web 版（claude.ai/code） |
|------|---------------|------------------------|
| 代码运行位置 | **你的本地机器** | Anthropic 的云端虚拟机 |
| 本地文件访问 | 完整的本地文件系统和 MCP 服务器 | 通过 Teleport 上传代码到云端 |
| 网络连接断开 | 机器重新上线后会话自动恢复 | 无影响（本就不在本地） |
| 适用场景 | 本地任务进行中，需要离开工位 | 不需要本地环境的独立任务 |

详细的 Remote Control CLI 标志、连接配置和故障排除，参见「第三方集成」笔记中的 [Channel 与其他集成方式对比](../integrations/index.md) 章节。
...
graph TD
  ...
  A -->|否| C{需要操控<br/>桌面 GUI？}
  C -->|是| D["Computer Use<br/>（macOS 研究预览）"]
  C -->|否| E{需要调试 Web 应用？}
  E -->|是| F["Chrome 扩展"]
  E -->|否| G{需要离开工位<br/>继续任务？}
  G -->|是| H["Remote Control"]
  G -->|否| I{需要环境隔离？}
  I -->|是| J["DevContainer"]
  I -->|否| K{你日常用什么 IDE？}
  K -->|VS Code / Cursor| L["VS Code 扩展"]
  K -->|IDEA / PyCharm| M["JetBrains 插件"]
  K -->|没有偏好 / 终端党| N{本地有开发环境吗？}
  N -->|有| O["终端 CLI"]
  N -->|没有 / 不在电脑旁| P["Web 版<br/>（claude.ai/code）"]
```

💡 **核心原则**：选你最顺手的平台就好。Claude Code 的能力是跨平台一致的，不存在"某个平台能用而另一个不能用"的功能。平台的区别只在于**交互体验**和**集成深度**——选择能让你最高效地描述需求、审阅结果的那一个。需要注意，Computer Use 目前仅限 macOS 研究预览，而 Remote Control 和 Chrome 扩展则支持更广泛的平台。
