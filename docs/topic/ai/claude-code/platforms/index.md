...
- **PowerShell Tool**：新增原生的 PowerShell 工具（2.1.111 新增，逐步推出），通过 `CLAUDE_CODE_USE_POWERSHELL_TOOL` 环境变量选择加入。启用后 Claude 可以直接在 PowerShell 中执行命令，无需 Bash 适配层，PowerShell 原生命令和管道语法都能原生使用
- **macOS Terminal.app 渲染修复**：修复了在不支持同步输出的终端（如 macOS Terminal.app）中启动时出现的渲染乱码问题（2.1.110 修复）
...
| 无闪烁模式 | 设置 `CLAUDE_CODE_NO_FLICKER=1` 环境变量可减少重绘闪烁；也可使用 `/tui fullscreen` 在会话中动态切换（2.1.110 新增） |
| 权限模式 | `Ask`（默认）/ `Auto accept edits` / `Plan` / `Auto` / `Bypass`；Auto 模式不再需要 `--enable-auto-mode` 参数（2.1.111 改进） |
| 会话恢复 | `/resume` 恢复历史会话 |
...
| Write 感知 | ❌ 不感知编辑器中的手动修改 | ✅ 在 diff 视图中编辑了建议内容后再接受时通知模型（2.1.110 改进） |
