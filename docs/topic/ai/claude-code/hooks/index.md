...
| `http` | 向 URL 发送 POST 请求 | 需要外部服务处理逻辑时（如审计日志、远程校验） |
...
| `InstructionsLoaded` | CLAUDE.md 或 rules 文件加载到上下文时 | ❌ 仅用于审计/可观测 |
| `CwdChanged` | 工作目录切换时 | ❌ 但可刷新环境变量 |
| `FileChanged` | 监听的文件发生变化时 | ❌ 但可刷新环境变量 |
...
| `PermissionRequest` | 权限对话框出现时 | ✅ 可自动批准或拒绝 |
...
| `TaskCreated` | 通过 TaskCreate 创建任务时 | ✅ 可阻止任务创建 |
...
| 事件 | 触发时机 | 能否阻止 |
|------|---------|---------|
| `ConfigChange` | 配置文件在会话中被修改时 | ✅ 可阻止配置变更生效 |
| `PreCompact` | 上下文压缩前 | ❌ |
