...
- 🔧 Claude Code 的 29 个生命周期事件分别什么时候触发
...
- HTTP 响应的状态码**不会**阻止操作。即使返回 4xx 或 5xx，操作也不会被阻断。要阻止工具调用，必须在响应体中返回 2xx 状态码 + 适当的 `hookSpecificOutput` 字段
...
Claude Code 的一个会话从开始到结束，经历了 29 个可注册 Hook 的生命周期事件。为了方便理解，我们可以把它们分成**四个阶段**。
...
Claude Code 会用修改后的参数替换原始参数，然后继续执行。注意：当多个 `PreToolUse` Hook 同时返回 `updatedInput` 时，由于 Hook 是并行运行的，最后完成的那一个会"赢"——这是一个非确定性的结果。所以应避免让多个 Hook 修改同一个工具的输入。
