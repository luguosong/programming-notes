...
- 📌 多层级覆盖规则：托管 > 用户 > 项目 > 插件，谁优先谁让路（优先级链：托管策略 > 用户级 > 项目级 > 插件）
...
> - 给他一份**员工手册**（`CLAUDE.md`，项目级持久上下文文件），告诉他项目规范和禁忌
> - 给他配备**专业工具**（`MCP`，Model Context Protocol，连接外部服务的协议），让他能连数据库、发 Slack 消息
> - 教他一些**标准操作流程**（`Skills`，可复用的知识库和工作流），比如「部署上线前要跑这几步检查」
...
💡 **新手上路建议**：先从 `CLAUDE.md` 开始——把项目规范写进去，立竿见影。等需求明确了，再按需添加其他扩展。推荐的渐进路径是：`CLAUDE.md`（规范）→ `Skill`（工作流）→ `Hook`（强制自动化）→ `MCP`（外部连接）→ `Plugin`（打包分发）。
...
此外还有一个**打包层**，负责把上述扩展机制打包、分发和安装。`Plugin` 和 `Channel` 在扩展体系中扮演的角色不同于其他六个机制——它们不直接提供新能力，而是把已有能力**打包成可分发的单元**：

- `Plugin`（插件）：把 Skills、Hooks、Subagents、MCP Server 等多种扩展组件打包成一个**可安装单元**。用户通过插件市场（`/plugin install`）一键安装，无需手动复制配置文件。插件中的 Skill 自带命名空间（如 `/my-plugin:review`），多个插件可以共存而不冲突。适合跨仓库复用或通过 marketplace 分享给团队和社区。详见「插件」。
- `Channel`（通道）：一种特殊的 MCP 服务器，作用是将外部事件（即时消息、webhook 回调、监控告警等）**主动推送**到正在运行的 Claude Code 会话中。与普通 MCP 的「Claude 主动查询」模式不同，Channel 是「外部主动通知 Claude」。Channel 以插件形式安装，通过 `--channels` 标志启用。详见「Channels」。
...
简单来说：`CLAUDE.md` 放全局规则，`.claude/rules/` 放条件规则（只在操作特定文件时生效），`Skill` 放按需使用的内容。
...
综合上述机制，Claude Code 扩展的完整优先级为：

**托管策略 > 用户级 > 项目级 > 插件**

- **托管策略**（managed policy）：由企业管理员通过托管配置文件统一部署，用户无法修改，具有最高优先级
- **用户级**（user）：`~/.claude/` 下的配置，对所有项目生效，适合个人通用偏好
- **项目级**（project）：项目 `.claude/` 下的配置，通过 Git 共享给团队，适合项目专属规范
- **插件**（plugin）：通过市场安装的扩展包，功能最丰富但优先级最低

⚠️ `CLAUDE.md` 是个例外——它是累加式的，所有层级的内容同时生效，不做覆盖。

### 环境变量：更底层的精细控制

除了上述扩展机制，Claude Code 还提供了 **230+ 个环境变量**用于精细控制行为。这些变量覆盖了 MCP 连接、工具搜索、输出限制、自动更新等多个方面。
...
| `MAX_MCP_OUTPUT_TOKENS` | 单个 MCP 工具输出的 token 上限 |
| `ENABLE_TOOL_SEARCH` | 控制工具搜索（Tool Search）行为 |
| `DISABLE_AUTOUPDATER` | 禁用 Claude Code 和插件的自动更新 |
| `CLAUDE_CODE_USE_BEDROCK` | 使用 AWS Bedrock 作为模型后端 |
| `CLAUDE_CODE_USE_VERTEX` | 使用 Google Vertex AI 作为模型后端 |
| `SLASH_COMMAND_TOOL_CHAR_BUDGET` | Skill 描述的字符预算上限 |
| `DISABLE_PROMPT_CACHING` | 禁用提示缓存（调试用） |
| `CLAUDE_BASH_MAINTAIN_WORKING_DIR` | 保持 Bash 工具的工作目录（默认为 `1`） |
