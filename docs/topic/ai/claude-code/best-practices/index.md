...
12. **调整 Prompt 缓存 TTL** → Claude Code 支持自定义 Prompt 缓存的生存时间（TTL）。默认情况下订阅用户享受 1 小时缓存 TTL，你也可以通过 `settings.json` 的 `env` 字段设置环境变量来调整：`ENABLE_PROMPT_CACHING_1H=1` 启用 1 小时缓存，`FORCE_PROMPT_CACHING_5M=1` 强制 5 分钟缓存。更长的缓存 TTL 意味着重复对话的成本更低（2.1.108 新增）。配置方式：
