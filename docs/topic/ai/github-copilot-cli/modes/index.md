---
title: 交互模式
description: 掌握 Interactive、Plan、Autopilot、Research、Fleet、Programmatic 六种交互模式的适用场景
---
...
- 🎛️ Copilot CLI 的多种交互模式及其适用场景
- ⚡ 每种模式的特点和进入方式
- 🎯 如何选择最适合当前任务的模式
- 📤 任务委派（`/delegate`）的使用方法

打个比方：`Interactive` 模式像是你坐在副驾驶，每一步都要你点头才能继续；`Plan` 模式像是先让导航规划路线，你看完路线再出发；`Autopilot` 模式像是告诉司机目的地，然后你靠在座位上等到达；`Research` 模式像是请了一位高级顾问，他不干活但会给你一份详尽的调研报告；`Fleet` 模式像是同时派出多个专员，各干各的活。

Copilot CLI 提供三种交互模式，通过 ++shift+tab++ 循环切换。状态栏左侧会显示当前所处的模式。此外还有 Research（通过 `/research` 触发）、Fleet（通过 `/fleet` 触发）和非交互式的 Programmatic 模式用于脚本集成。
...
=== "命令行启动"
    ...
    - `--yolo`：跳过所有权限确认
    - `--max-autopilot-continues N`：限制最大自动续行次数（默认 5 次，1.0.40 新增默认限制）
...
!!! warning "Autopilot 续接限制（1.0.40 新增）"

    Autopilot 模式默认将续接消息限制为 5 次，防止任务失控导致无限循环。如果你的任务需要更多轮次，可以通过 `--max-autopilot-continues` 参数调整上限。
...
Research 模式就像请了一个高级顾问——他不直接动手改代码，而是深入调研后给你一份详尽的分析报告。

### 触发方式

Research Agent 通过 `/research` 命令触发，**无法自动触发**：

``` text
/research 这个项目的认证架构有哪些安全风险？
```

!!! warning "模型不可切换"

    Research Agent 使用硬编码的专用模型，无法通过 `/model` 命令切换。无论你的主会话选择了什么模型，Research Agent 始终使用其内置的模型。

### 查询类型自适应

Research Agent 会自动将你的问题分类为三种类型，并据此调整报告格式：

| 查询类型 | 说明 | 报告格式 |
|----------|------|----------|
| 流程问题 | "如何从零搭建 OAuth2？" | 分步指导 + 相关文档链接 |
| 概念问题 | "React Server Components 和普通组件有什么区别？" | 叙述性解释 + 对比分析 + 设计决策 |
| 技术深度分析 | "这个项目的认证架构有哪些安全风险？" | 架构图 + 组件拆解 + 代码示例 + 引用来源 |

!!! tip "控制报告类型"

    你可以通过调整提问方式来引导 Agent 的分类判断。例如，如果你想要一份技术深度报告，但用了"X 是什么？"这样的概念问法，可能会得到概念层面的回答。试试改成："对 X 做技术深度分析，包含架构图和代码示例。"

### 报告的查看与分享

研究完成后，Copilot 会显示关键发现的摘要，并给出完整报告的 Markdown 文件链接。你也可以按 ++ctrl+y++ 在终端中直接查看当前会话最近的研究报告。
...
!!! info "报告存储位置"
    ...
    报告保存在会话特定的目录中（`~/.copilot/session-state/SESSION-ID/research/`）。新会话中无法通过 ++ctrl+y++ 或 `/share` 访问旧报告，但可以直接在文件系统中找到它们。

### 适用场景

- 需要深入理解的技术问题或架构分析
- 技术选型前的全面调研（如"JWT 和 Session 认证方案的对比"）
- 代码库结构梳理（如"这个项目的架构是怎样的？"）
- 内部实现模式追踪（如"我们组织的 Feature Flag 是如何实现的？"）

### 不适用场景

- **快速问答**："这个函数是干什么的？"——普通 Chat 更快
- **需要修改代码**：Research 只产出报告，不会改代码
- **时间敏感的交互**：研究过程需要大量工具调用，耗时较长

### 示例提示词

``` text
/research 这个代码库的整体架构是什么？
```

``` text
/research React 是如何实现并发渲染的？
```

``` text
/research 如何给这个项目的 API 添加一个新的端点？
```

``` text
/research 这个仓库中的 Session 管理系统是如何实现的？
```
...
Fleet 模式就像同时派出多个专员并行处理不同子任务——每个专员有独立的记忆空间，互不干扰。
...
=== "斜杠命令"
    ...
    ``` text
    /fleet 按照之前制定的计划实施所有变更
    ```

=== "Plan 审批后触发"

    在 Plan 模式中完成计划后，选择 **Accept plan and build on autopilot + /fleet**，Copilot 会自动使用子代理并行执行。

### 工作原理

当你使用 `/fleet` 命令时，主 Copilot Agent 会分析 prompt，判断是否可以拆解为更小的独立子任务。如果可以，它会充当**编排者（Orchestrator）**的角色：

- 将大任务拆解为多个独立子任务
- 为每个子任务分配独立的子代理（Sub-agent）
- 每个子代理拥有**独立的上下文窗口**，专注于自己的任务
- 各子代理尽可能并行执行，加速整体完成

```mermaid
graph LR
    A["/fleet 命令"] --> B["主 Agent<br/>编排者"]
    B --> C["子代理 1"]
    B --> D["子代理 2"]
    B --> E["子代理 3"]
    C --> F["结果汇总"]
...
!!! tip "最佳实践：先 Plan 后 Fleet"

    最高效的工作流是先在 Plan 模式中制定详细计划，确认后再选择 **Accept plan and build on autopilot + /fleet**。这样既能保证方向正确，又能最大化并行执行效率。

### 监控并行子代理

使用 `/tasks` 命令查看当前会话中所有后台任务，包括 Fleet 的子代理任务：

| 操作 | 快捷键 | 说明 |
|------|--------|------|
| 查看子代理详情 | ++enter++ | 查看子任务详情，完成后可看到执行摘要 |
| 终止子代理 | ++k++ | 强制终止正在运行的子代理 |
| 移除已完成任务 | ++r++ | 从列表中移除已完成或已终止的子任务 |
| 退出任务列表 | ++esc++ | 返回主 CLI 提示符 |

### 任务组合要求

Fleet 模式要发挥作用，子任务必须满足以下条件：

- **可独立执行**：子任务之间没有依赖关系
- **无数据竞争**：不同子代理不会修改同一文件
- **适合并行化**：如为不同模块编写测试、更新多个独立文件等

!!! warning "高级请求消耗"

    每个子代理会独立与 LLM 交互，因此使用 `/fleet` 可能比由单个 Agent 处理相同任务消耗**更多 Premium Requests**。如果子代理使用了自定义 Agent 配置中指定的特定模型，将按该模型的倍率计费。使用 `/model` 命令可查看当前模型及其倍率。

### 适用场景

- **大型任务**：涉及多个独立步骤的请求，如多文件重构、跨模块依赖更新
- **可并行化的工作**：如为不同模块编写测试套件
- **自动化工作流**：搭配 Autopilot 实现最快的任务完成速度

### 与 Autopilot 的关系

Fleet 和 Autopilot 是两个独立的特性，但经常搭配使用：

- **Autopilot**：让 Copilot 自主连续执行，直到任务完成
- **Fleet**：用子代理并行执行子任务，加速整体完成

你可以在 Interactive 模式中单独使用 `/fleet`，也可以在 Autopilot 中自动调用它。最典型的组合方式是：Plan 模式制定计划 → 选择 "Accept plan and build on autopilot + /fleet" → Copilot 自主并行完成所有实施步骤。
...
!!! warning "Prompt 模式安全门控（1.0.40 新增）"

    在 Prompt 模式（`-p`）下，出于安全考虑，默认禁止加载仓库 Hook 和工作区 MCP 服务器——防止在 CI 或自动化场景中意外执行未经审核的本地代码。如果确实需要启用，需通过环境变量显式开启：

    - `GITHUB_COPILOT_PROMPT_MODE_REPO_HOOKS=1`：启用仓库 Hook
    - `GITHUB_COPILOT_PROMPT_MODE_WORKSPACE_MCP=1`：启用工作区 MCP 服务器
...
| 特性 | Interactive | Plan | Autopilot | Research | Fleet | Programmatic |
|------|:-----------:|:----:|:---------:|:--------:|:-----:|:------------:|
| 每步需要确认 | ✅ | ✅ | ❌ | N/A | 部分 | N/A |
| 先规划后执行 | ❌ | ✅ | ❌（可配合 Plan） | N/A | ✅（推荐） | ❌ |
| 自主连续执行 | ❌ | ❌ | ✅ | ✅ | ✅ | ❌ |
| 并行子代理 | ❌ | ❌ | 可选 | ❌ | ✅ | ❌ |
| 产出物 | 代码变更 | 实现计划 | 代码变更 | 调研报告 | 代码变更 | 代码/文本 |
| 适合场景 | 日常编码、调试 | 复杂多文件变更 | 明确的大型任务 | 技术调研、选型 | 可并行的大任务 | 脚本/自动化 |
| 进入方式 | 默认 | ++shift+tab++ / `/plan` | ++shift+tab++ | `/research` | `/fleet` | `copilot -p` |
