...
当你的 MCP 服务器部署在无头环境（如 CI 服务器）中、无法打开浏览器时，`client_credentials` OAuth 授权类型可以直接使用客户端凭据完成认证，无需任何浏览器交互（1.0.40 新增）。此前，MCP OAuth 只支持需要浏览器回调的授权码流程和设备码流程，在纯服务器环境中不太方便。此外，当多个 MCP 服务器共享相同 URL 但使用不同静态 OAuth 客户端 ID 时，OAuth 令牌现在能正确缓存（1.0.40 修复），避免了之前因缓存冲突导致认证失败的问题。
