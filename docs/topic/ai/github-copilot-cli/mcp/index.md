...
| DevContainer | `.devcontainer/devcontainer.json`（1.0.3 新增） | 开发容器环境 |
...
MCP 服务器可以请求 LLM 推理能力（sampling），让服务器端逻辑借助 AI 做出更智能的决策（1.0.13 新增）。出于安全考虑，每次 sampling 请求需要通过审查提示获得用户批准。

---

## 🛡️ MCP 安全策略

- **Allowlist 验证**：MCP 服务器可通过 `MCP_ALLOWLIST` 对配置的注册表进行验证，确保只加载受信任的服务器（1.0.8 新增）
- **组织级策略**：组织级第三方 MCP 服务器策略对所有用户强制执行（1.0.11 改进）
- **Workspace 信任**：Workspace MCP 服务器在用户确认文件夹信任后才加载（1.0.10 改进）
