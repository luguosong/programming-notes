---
title: Copilot CLI+Claude Code双工具协同实践
...
三文件同步 和 Monorepo 分层是同一套体系在不同规模下的应用——前者是简单场景，后者是复杂场景。
...
``` bash title="scripts/check-ai-config.sh"
#!/bin/bash
# 检查双工具配置是否存在常见冗余
set -e
