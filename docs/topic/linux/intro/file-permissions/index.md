...
想象公司资料室的文件柜：老板（Owner）有全部钥匙可以访问所有柜子；项目组成员（Group）只有项目文件柜的钥匙；其他员工（Others）只能看公告栏（无限制的公开区）。Linux 文件权限的设计思想完全一样——通过三类不同的权限主体，灵活控制谁能读、写、执行文件。

## 三类权限主体：谁在操控文件？

Linux 的每个文件都同时面向三类对象设置权限：

**🤴 文件拥有者（User / Owner）** — **谁创建了这个文件？**  
创建文件的账号。拥有者对自己的文件拥有最高控制权，可以独立改变权限或转移所有权。

**👥 所属群组（Group）** — **多个人要共享一个文件怎么办？**  
文件关联的群组，群组内所有成员共享同一套权限。典型使用场景是团队协作——把同一项目的成员加入同一群组，就能让他们互相读写项目文件，而其他人看不到。这比逐个设置每个人的权限效率高得多。

**🌐 其他人（Others）** — **这个文件对陌生人有什么限制？**  
既不是拥有者、也不在群组内的所有账号。这是"默认限制"——任何系统用户在没有明确被授予权限的情况下，只能访问 Others 权限允许的操作。
...
下面是权限信息最直观的呈现方式。通过 `ls -l` 命令，你可以一眼看清每个文件"属于谁、谁能做什么"。
