...
服务器运维中，你经常需要快速了解系统当前状态——时间对不对？系统跑了多久？硬件是 x86 还是 ARM？这一节介绍六个最常用的"一眼看状态"命令。

### date — 查看与设置系统时间

当脚本需要按日期归档日志、或者发现服务器时间与本地时间对不上时，`date` 命令是你的第一选择。

``` bash title="date 常用示例"
# 显示当前系统时间
date
...
!!! warning "修改系统时间需谨慎"
    ...
    `date -s` 直接修改系统时钟，只有 `root` 才能执行。生产环境建议使用 `timedatectl` 配合 NTP 自动同步时间，避免手动调整造成时间跳变。

### cal — 查看日历

想快速确认某个月从星期几开始，或查看某年的日历安排？`cal` 比翻手机日历更快。
...
Shell 脚本里做浮点运算是个痛点——Bash 原生只支持整数。`bc`（Basic Calculator）填补了这个空缺，支持任意精度、进制转换，还能在管道中直接使用，是脚本数学运算的利器。
...
``` bash title="bc 管道模式（脚本中常用）"
# 直接计算（管道传入表达式）
echo "2.5 * 3.14" | bc

# 设置小数精度（scale=4 表示保留4位小数，默认只保留整数）
echo "scale=4; 22/7" | bc

# 进制转换：十进制 255 转十六进制（obase = 输出进制）
echo "obase=16; 255" | bc

# 进制转换：十六进制 FF 转十进制（ibase = 输入进制）
echo "ibase=16; obase=10; FF" | bc
```

!!! note "scale 参数"

    `bc` 默认做整数除法（`22/7` 结果是 `3`）。在脚本中务必先设置 `scale=N`，否则精度会静默丢失。

### uptime — 系统运行时长与负载

登录新机器时，第一个问题往往是"这台机器跑了多久？负载正常吗？"——`uptime` 一行输出全部告诉你。

``` bash title="uptime 示例输出"
uptime
# 14:23:05 up 103 days, 6:13,  3 users,  load average: 1.90, 1.49, 1.38
```

输出字段含义：

- `14:23:05`：当前时间
- `up 103 days, 6:13`：系统已连续运行 103 天 6 小时 13 分钟
- `3 users`：当前登录的用户数
- `load average: 1.90, 1.49, 1.38`：过去 1 分钟、5 分钟、15 分钟的平均负载

!!! note "如何判断负载是否正常"

    负载值表示"等待 CPU 执行的任务队列长度"。单核 CPU 上，负载 `1.0` 表示恰好满载；多核 CPU 上，参考值等于核心数——4 核机器负载 `4.0` 才算满载，`2.0` 只用了一半。用 `nproc` 或 `lscpu` 查看核心数。

### uname — 查看系统内核信息

写需要判断架构的脚本（如 x86 与 ARM 分支下载不同二进制），或排查内核相关问题时，`uname` 提供关键信息。

``` bash title="uname 常用示例"
# 显示所有系统信息（内核名、主机名、内核版本、构建时间、硬件平台）
uname -a

# 只显示内核版本号（脚本中最常用）
uname -r

# 显示硬件架构（x86_64 / aarch64 / armv7l 等）
uname -m
```

``` bash title="uname -a 示例输出"
Linux myserver 5.15.0-107-generic #117-Ubuntu SMP Fri Apr 26 12:14:31 UTC 2024 x86_64 x86_64 x86_64 GNU/Linux
```

字段依次为：内核名（`Linux`）、主机名、内核版本、构建时间、硬件平台、CPU 类型、指令集、操作系统名。

### hostname — 查看与修改主机名

管理多台服务器时，区分机器靠的就是主机名。`hostname` 显示或临时修改当前主机名，但要永久生效需要修改配置文件或使用 `hostnamectl`。

``` bash title="hostname 查看主机名"
# 显示当前主机名
hostname
```

现代 Linux 系统（systemd）推荐用 `hostnamectl` 永久修改，避免重启后失效：

``` bash title="hostnamectl 永久修改主机名（推荐）"
# 查看主机名详情（包括静态名称、图标类型等）
hostnamectl
...
!!! note "主机名配置文件"
    ...
    无论 Debian/Ubuntu 还是 Red Hat/RHEL，现代发行版都使用 `/etc/hostname` 存储主机名。`hostnamectl set-hostname` 会自动修改此文件，无需手动编辑。

---

## 👥 用户状态查询

多用户场景下，你需要知道"谁在线"、"他们在做什么"、"上次登录是什么时候"。这四个命令从不同维度回答这些问题。

### who — 查看当前登录用户

当你 SSH 进入服务器，想确认自己所在的终端编号，或者查看是否有其他用户同时在线，`who` 是最直接的工具。

``` bash title="who 常用示例"
# 显示所有当前登录的用户
who

# 只显示自己的登录信息（SSH 多跳时确认当前身份）
who am i
...
``` bash title="who 示例输出"
tammy    tty1         2025-01-11 21:25
root     tty2         2025-01-11 15:12
alice    pts/0        2025-01-11 10:07 (192.168.1.100)
```

输出字段：用户名、终端名（`tty` 表示本地终端，`pts` 表示伪终端/SSH 会话）、登录时间、登录来源 IP（远程登录才显示）。

### w — 用户活动详情

`who` 只告诉你"谁在"，`w` 进一步告诉你"他们在做什么、占用多少 CPU、已经空闲多久"——相当于 `who` 的增强版。

``` bash title="w 常用示例"
# 查看所有用户的活动状态（第一行同 uptime）
w
...
``` bash title="w 示例输出"
 14:23  up 9 days,  7:02,  3 users,  load average: 0.11, 0.02, 0.00
USER     TTY      FROM          LOGIN@   IDLE JCPU   PCPU WHAT
alice    pts/0    192.168.1.10  10:07    0:00 0.01s  0.01s w
bob      pts/1    192.168.1.20  09:30   20:18 43.26s  1.52s vim notes.txt
```
...
- `LOGIN@`：登录时间
- `IDLE`：上次操作距今的空闲时长
- `JCPU`：该终端累计使用的 CPU 时间（J = Jobs，所有进程总和）
- `PCPU`：当前进程使用的 CPU 时间（P = Process）
- `WHAT`：当前正在执行的命令

### last — 查看登录历史

当服务器出现异常，或者需要审计"过去谁登录过系统"，`last` 能从系统日志中还原登录记录。

``` bash title="last 常用示例"
# 显示所有用户的登录/登出历史（来自 /var/log/wtmp）
last
...
# 查看登录失败记录（需要 root，来自 /var/log/btmp）
sudo lastb
```

!!! warning "lastb 需要 root 权限"

    `lastb` 记录的是**认证失败**的尝试，常用于排查暴力破解攻击。普通用户无法读取 `/var/log/btmp`，需要 `sudo lastb` 或切换到 root 执行。

### id — 显示用户身份信息

当你不确定当前用户的 UID/GID，或需要确认某用户是否属于某个组（例如 `sudo` 组），`id` 是最直接的方式。
...
- `uid=1000(alice)`：用户 ID 为 1000，用户名为 alice
- `gid=1000(alice)`：主组 ID 为 1000
- `groups=...`：alice 同时属于 `adm`（可读系统日志）、`sudo`（有 sudo 权限）、`docker`（可操作 Docker）等附加组

!!! tip "快速判断是否有 sudo 权限"

    运行 `id` 后，检查 `groups` 中是否包含 `sudo`（Debian/Ubuntu）或 `wheel`（Red Hat/RHEL）。包含则说明该用户可以使用 `sudo`。
