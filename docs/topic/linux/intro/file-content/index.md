...
cat > newfile.txt << EOF  # 从 Here Document 创建文件
Hello
World
EOF
```
...
| `-b` | 显示行号（空行不编号） |
| `-s` | 合并连续空行（多个空行变成一个） |
| `-A` | 显示所有不可见字符（包括行尾 `$` 和 Tab `^I`） |
| `-E` | 显示行尾 `$`（仅行末） |
| `-T` | 显示 Tab 为 `^I` |
| `-v` | 显示不可打印字符为 `^X` 格式 |

``` bash
# 调试：查看文件中是否有 Tab、换行符等隐藏字符
cat -A config.conf
...
`tac` 是 `cat` 的反向版本，从末尾第一行开始输出。

``` bash
tac /var/log/syslog | head    # 查看最后 10 条日志
...
`nl` 相比 `cat -n` 更灵活，支持更多格式化选项。

``` bash
# 空行也编号，行号右对齐补零，宽度 3 位
nl -ba -nrz -w3 file.txt

# 输出示例：
# 001 first line
# 002 second line
# 003
# 004 after blank line
```
